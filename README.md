...
1. 在 [Releases](https://github.com/peterL1244/AutoExtractor/releases) 下载 `AutoExtractor-v0.1.0-win-x64.zip`。
2. 解压整个便携包，保留 `tools` 文件夹及其他随附文件，双击 `AutoExtractor.exe`。
3. 将同一软件的所有分卷放到一个文件夹，拖入文件或文件夹。
4. 查看识别结果和“原名称 → 还原名称”，取消勾选想跳过的任务，点击“开始解压”。加密包会弹出密码框。
5. 完成后点击“打开结果”。智能停止或待确认的内层文件可用“继续解压内层”添加到任务列表。
...
- **原地改名**：成功验证归档后，在下载目录恢复正确后缀，原始内容不改变。
- 改名记录保存在原目录的 `.autoextractor-journals` 中。点击“恢复原名”，选择对应 JSON 记录；内容变化或名称被占用时会拒绝恢复，避免覆盖。
...
便携版输出到 `artifacts/AutoExtractor-win-x64`，发布 ZIP 与 SHA-256 清单输出到 `artifacts`。GitHub Actions 在 main 推送和 PR 时测试并打包，版本标签触发 Release 发布。
